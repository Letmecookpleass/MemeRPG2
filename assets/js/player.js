...
const playerLvlUp = () => {
    ...
    let expMaxIncrease = Math.floor(((player.exp.expMax * 1.3) + 500) - player.exp.expMax);
    if (player.lvl > 500) {
        expMaxIncrease = 10000000;
    }
    ...
    player.bonusStats.hp += 50;
    player.bonusStats.atk += 8;
    player.bonusStats.def += 7;
    player.bonusStats.atkSpd += 0.45;
    player.bonusStats.critRate += 0.4;
...
const playerLoadStats = () => {
    ...
    <p><i class="ra ra-dripping-blade"></i>LIFE STEAL+ ${player.bonusStats.vamp.toFixed(2).replace(rx, "$1")}%</p>
