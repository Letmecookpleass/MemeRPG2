...
const playerAttack = () => {
    ...
    if (player.skills.includes("Sigma")) {
    ...
    addCombatLog(`this fucking ${player.name} piss and dealt ` + nFormatter(damage) + ` ${dmgtype} to ${enemy.name}.`);
    hpValidation();
    playerLoadStats();
    enemyLoadStats();
...
const enemyAttack = () => {
    ...
    // Skill effects
    if (player.skills.includes("Spaghetti Soy Sauce Enjoyer")) {
    ...
    // Aegis Thorns skill
    objectValidation();
    if (player.skills.includes("Asian")) {
